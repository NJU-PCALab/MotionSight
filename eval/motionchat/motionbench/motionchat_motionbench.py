...
def parse_args():
    parser = argparse.ArgumentParser(description='Parallel evaluation of MotionChat video dataset')
    ...
    parser.add_argument('--data_file', type=str,
                        default='/path/to/MotionBench/video_info.meta.jsonl',
                        help='Path to data file')
    parser.add_argument('--output_filename', type=str, default="motionchat_qwen_parallel.jsonl",
                        help='Output filename')
    parser.add_argument('--resume', type=int, default=0,
                        help='Index to resume evaluation from')
    parser.add_argument('--restore_name', type=str, default='restore',
...
def process_chunk(gpu_id, data_chunk, args, output_lock, temp_output_path):
    ...
    print(f"Process {gpu_id} starting work on CUDA:{gpu_id}, processing {len(data_chunk)} videos")
    ...
    for i, item in enumerate(tqdm(data_chunk, desc=f"GPU {gpu_id} processing videos")):
        value_list = []
        print(f"GPU {gpu_id} processing: {item}")
        ...
        for q in item['qa']:
            ...
            try:
                mllm_response = run(model, processor, video_path, question,
                ...
                print(f"GPU {gpu_id} - Question: {question}")
                out = {'question':q['question'], 'question_type':item['question_type'],
                       'correct_answer':answer, 'output':mllm_response, 'judge':judge}
                ...
                out = {'question':q['question'], 'question_type':item['question_type'],
                       'correct_answer':answer, 'output':f"ERROR: {str(e)}", 'judge':False}
                value_list.append(out)
...
def main():
    ...
    # Read data
    data = read_video_info(args.data_file)
    if args.resume > 0:
    ...
    try:
        ...
        print(f"Parallel processing encountered an error: {e}")
